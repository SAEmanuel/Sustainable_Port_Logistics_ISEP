...
import CompleteVVEModal from './CompleteVVEModal';
...
export default function VesselVisitExecutionPage() {
  ...
  const [vveToComplete, setVveToComplete] = useState<VesselVisitExecutionExtended | null>(null);

  // ✅ RBAC / Read-only rules:
  // - If VVE status is Completed => read-only for non-admin
  const isAdmin = (user?.role ?? "") === Roles.Administrator;
  const isVveCompleted = (v?: { status?: string | null }) =>
    String(v?.status ?? "").toLowerCase() === "completed";
  const isReadOnlyVve = (v?: { status?: string | null }) =>
    isVveCompleted(v) && !isAdmin;
  ...
  const openEditBerthModal = async () => {
    ...
    if (isReadOnlyVve(selectedHistoryItem)) {
      notifyError("Visita concluída: só um Administrador pode editar.");
      return;
    }
  ...
  const openCompleteModal = (vve: VesselVisitExecutionExtended) => {
    // Completed => no need; read-only for non-admin
    if (isVveCompleted(vve)) return;
    if (isReadOnlyVve(vve)) {
      notifyError("Visita concluída: só um Administrador pode editar.");
      return;
    }
    setVveToComplete(vve);
  ...
  const toIsoOrUndef = (x: unknown) => {
    if (!x) return undefined;
    if (x instanceof Date) return x.toISOString();
    if (typeof x === "string") return x; // assume ISO
    return String(x);
  };

  const normalizeVveTimes = <T extends Record<string, any>>(obj: T) => ({
    ...obj,
    actualArrivalTime: toIsoOrUndef(obj.actualArrivalTime),
    actualBerthTime: toIsoOrUndef(obj.actualBerthTime),
    actualUnBerthTime: toIsoOrUndef(obj.actualUnBerthTime),
    actualLeavePortTime: toIsoOrUndef(obj.actualLeavePortTime),
    updatedAt: toIsoOrUndef(obj.updatedAt),
  });
    ...
    <Container size="xl" py="xl" style={{ backgroundColor: '#f4f7f6', minHeight: '100vh' }}>
                ...
                <Grid.Col span={{ base: 12, md: 6, lg: 4 }} key={vve.id}>
                  ...
                  >
                    <Group justify="space-between" mb="xs">
                      <Badge variant="light" color={isVveCompleted(vve) ? "gray" : "teal"} size="lg" radius="sm">
                        {isVveCompleted(vve) ? "Completed" : t('vesselVisitExecution.inPort')}
                      </Badge>
                    </Group>
          ...
          <Stack gap="md">
              ...
              <Group justify="space-between" align="start">
                <Group>
                  ...
                  <div>
                    ...
                    {isReadOnlyVve(selectedHistoryItem) ? (
                      <Text size="xs" c="dimmed" mt={6}>
                        Modo só leitura (apenas Administrador pode editar).
                      </Text>
                    ) : null}
                  </div>
                </Group>

                <Badge color={isVveCompleted(selectedHistoryItem) ? "gray" : "teal"} variant="filled">
                  {isVveCompleted(selectedHistoryItem) ? "COMPLETED" : "IN PROGRESS"}
                </Badge>
              </Group>
            </Paper>
            ...
            <Stack gap={6}>
              <Group justify="space-between" align="center">
                ...
                <Button
                  size="xs"
                  variant="light"
                  onClick={() => void openEditBerthModal()}
                  disabled={isReadOnlyVve(selectedHistoryItem)}
                >
                  Atualizar
                </Button>
              </Group>
            ...
            <Group justify="flex-end" mt="md">
              <Button
                color="green"
                size="sm"
                onClick={() => selectedHistoryItem && openCompleteModal(selectedHistoryItem)}
                disabled={
                  !selectedHistoryItem ||
                  isVveCompleted(selectedHistoryItem) ||
                  isReadOnlyVve(selectedHistoryItem)
                }
              >
                Complete VVE
      ...
      >
        {!selectedHistoryItem?.id || !selectedHistoryItem?.vvnId ? (
          ...
          <ExecutedOperationsEditor
            ...
            // Se quiseres readOnly no editor, cria a prop lá e passa-a:
            // readOnly={isReadOnlyVve(selectedHistoryItem)}
          />
        )}
      </Modal>
      ...
      >
        <Stack>
          ...
          <TimeInput
            ...
            withSeconds={false}
            disabled={isReadOnlyVve(selectedHistoryItem ?? undefined)}
            leftSection={
              <ActionIcon
                variant="subtle"
                color="teal"
                onClick={() => {
                  if (isReadOnlyVve(selectedHistoryItem ?? undefined)) return;
          ...
          <Select
            ...
            disabled={loadingDockOptions || isReadOnlyVve(selectedHistoryItem ?? undefined)}
      ...
      {vveToComplete && (
        <CompleteVVEModal
          ...
          vve={vveToComplete as any}
          updaterEmail={user?.email ?? ""}
          onCompleted={(updated) => {
            const norm = normalizeVveTimes(updated as any);

            setHistory((prev) =>
              prev.map((h) => (h.id === norm.id ? ({ ...h, ...norm } as any) : h))
            );

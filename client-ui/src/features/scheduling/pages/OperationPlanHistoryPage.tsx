...
import type { OperationPlanFilterDTO, SaveScheduleDto, SchedulingOperationDto } from "../dtos/scheduling.dtos";
import { notifyError } from "../../../utils/notify";

import OperationPlanUpdateModal from "../components/OperationPlanUpdateModal";

type HistoryPlan = SaveScheduleDto & {
    domainId?: string;
    _id?: string;
    totalDelay?: number;
    planDate: any; // backend pode vir como Date/string
};

type SortField = "planDate" | "algorithm" | "author" | "total_delay";
    ...
    return (
        <Tooltip label={getFullDateString()} color="dark" withArrow transitionProps={{ duration: 200 }}>
            <Group gap={6} style={{ cursor: "help" }}>
                ...
                {daysToAdd > 0 && (
                    <Badge size="xs" variant="filled" color="orange" radius="sm" tt="lowercase">
                        +{daysToAdd}d
                    </Badge>
                )}
            </Group>
...
function safePlanId(p: HistoryPlan, index: number) {
    return p.domainId || (p as any)._id || `plan-${index}`;
}

function toDateValue(d: any): number {
    if (!d) return 0;
...
function getDelay(plan: any): number {
    const v = plan?.total_delay ?? plan?.totalDelay ?? plan?.totalDelay ?? 0;
    const n = Number(v);
...
export default function OperationPlanHistoryPage() {
    ...
    // sorting
    const [sortBy, setSortBy] = useState<SortField>("planDate");
    const [reverseSortDirection, setReverseSortDirection] = useState(true);

    // modal update
    const [editOpened, setEditOpened] = useState(false);
    const [selectedPlan, setSelectedPlan] = useState<HistoryPlan | null>(null);
    ...
    const fetchHistory = async (overrideFilters?: OperationPlanFilterDTO) => {
        ...
        try {
            ...
            if (!overrideFilters) {
                if (vesselFilter?.trim()) filters.vessel = vesselFilter.trim();
            ...
            // normaliza para o tipo que a página usa
            setPlans((data as any[]).map((p) => p) as HistoryPlan[]);
        } catch (e) {
            console.error(e);
    ...
    const sortedPlans = useMemo(() => {
        ...
        const pick = (p: HistoryPlan) => {
            if (sortBy === "planDate") return toDateValue(p.planDate);
            if (sortBy === "total_delay") return getDelay(p);
            if (sortBy === "algorithm") return (p.algorithm || "").toLowerCase();
            if (sortBy === "author") return (p.author || "").toLowerCase();
            return 0;
        };
    ...
    const formatDate = (dateStr: any) => {
        const ts = toDateValue(dateStr);
        if (!ts) return "-";
        return new Date(ts).toLocaleDateString();
    };

    const openEdit = (plan: HistoryPlan) => {
        setSelectedPlan(plan);
        setEditOpened(true);
    };

    const handleUpdated = async () => {
        // mais robusto: refetch com os filtros atuais (garante consistência)
        await fetchHistory();
    };
                    ...
                    <Group gap="md">
                        ...
                        <Stack gap={0}>
                            <Title order={2} c="dark.7">
                                {t("planningScheduling.historyTitle") || "Histórico de Planos"}
                            </Title>
                            <Text size="sm" c="dimmed">
                                Consulte os agendamentos passados
                            </Text>
                        </Stack>
                    </Group>
                </Group>

                {/* MODAL (fora da tabela, separado) */}
                <OperationPlanUpdateModal
                    opened={editOpened}
                    onClose={() => setEditOpened(false)}
                    plan={selectedPlan}
                    onUpdated={handleUpdated}
                    // token={...} se tiveres JWT
                />

                {/* Filtros */}
                <Card withBorder radius="md" shadow="sm" p="lg" bg="white">
                    <Text fw={600} mb="sm" size="sm" tt="uppercase" c="dimmed">
                        Filtros de Pesquisa
                    </Text>
                    ...
                    <Grid align="flex-end" gutter="md">
                        ...
                        <Grid.Col span={{ base: 12, md: 3 }}>
                            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                                <Text size="sm" fw={500}>
                                    {t("planningScheduling.startDate") || "Data Início"}
                                </Text>
                                <input
                                    type="date"
                        ...
                        <Grid.Col span={{ base: 12, md: 3 }}>
                            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                                <Text size="sm" fw={500}>
                                    {t("planningScheduling.endDate") || "Data Fim"}
                                </Text>
                                <input
                                    type="date"
                                    className="date-input-modern"
                    ...
                    <Table verticalSpacing="md" highlightOnHover>
                        <Table.Thead bg="gray.1">
                            <Table.Tr>
                                ...
                                <Th sorted={sortBy === "planDate"} reversed={reverseSortDirection} onSort={() => setSorting("planDate")}>
                                ...
                                >
                                    {t("planningScheduling.total_Delay") || "Total Delay"}
                                </Th>

                                <Th sorted={sortBy === "author"} reversed={reverseSortDirection} onSort={() => setSorting("author")}>
                                    {t("planningScheduling.author") || "Gerado Por"}
                                </Th>

                                <Table.Th>{t("planningScheduling.status2") || "Status"}</Table.Th>
                        ...
                        <Table.Tbody>
                            {loading ? (
                                ...
                                <Table.Tr>
                                    <Table.Td colSpan={8}>
                                        <Center py={50} style={{ flexDirection: "column" }}>
                                ...
                                sortedPlans.map((plan, index) => {
                                    const uniqueKey = safePlanId(plan, index);
                                    const isOpen = openedPlan === uniqueKey;

                                    const delayValue = getDelay(plan);
                                    const isOk =
                                        plan.status?.toLowerCase() === "ok" ||
                                    ...
                                    return (
                                        <React.Fragment key={uniqueKey}>
                                            <Table.Tr
                                                ...
                                                style={{
                                                    cursor: "pointer",
                                                    backgroundColor: isOpen ? "var(--mantine-color-blue-0)" : undefined,
                                                }}
                                            >
                                                <Table.Td>
                                                ...
                                                <Table.Td fw={600} style={{ fontSize: "1.05rem" }}>
                                                ...
                                                <Table.Td style={{ textAlign: "center" }}>
                                                    <Badge color="gray" variant="outline">
                                                        {(plan.operations as any[])?.length || 0}
                                                    </Badge>
                                                </Table.Td>
                                                ...
                                                <Table.Td>
                                                    <Group gap={6}>
                                                        ...
                                                        <Text fw={700} c={delayValue > 0 ? "red.8" : "green.8"}>
                                                            {delayValue}h
                                                        </Text>
                                                    </Group>
                                                </Table.Td>
                                                ...
                                                <Table.Td>
                                                    <Group gap="xs">
                                                        ...
                                                        <Tooltip label={plan.author || "System"} withArrow>
                                                            <Text size="sm" c="dimmed" style={{ cursor: "help" }}>
                                                                {(plan.author || "System").split("@")[0]}
                                                            </Text>
                                                        </Tooltip>
                                                    </Group>
                                                </Table.Td>
                                                    ...
                                                    <Group gap={8}>
                                                        <Box
                                                            style={{
                                                                ...
                                                                boxShadow: isOk
                                                                    ? "0 0 8px var(--mantine-color-green-4)"
                                                                    : "0 0 8px var(--mantine-color-orange-4)",
                                                            }}
                                                        />
                                                        <Text
                                                            size="sm"
                                                            fw={600}
                                                            c={isOk ? "green.8" : "orange.8"}
                                                            tt="uppercase"
                                                ...
                                                <Table.Td style={{ textAlign: "right" }}>
                                                    <Button
                                                        ...
                                                        variant="light"
                                                        onClick={(e) => {
                                                            e.stopPropagation(); // não abre/fecha o collapse
                                                            openEdit(plan);
                                                        }}
                                                    >
                                                        Editar
                                            ...
                                            <Table.Tr style={{ display: isOpen ? "table-row" : "none" }}>
                                                <Table.Td colSpan={8} p={0} style={{ borderBottom: isOpen ? "1px solid #dee2e6" : "none" }}>
                                                    <Collapse in={isOpen}>
                                                                    ...
                                                                    <Table.Tbody>
                                                                        {(plan.operations as SchedulingOperationDto[] | undefined)?.map((op: any, idx: number) => (
                                                                            <Table.Tr key={idx}>
                                                                                ...
                                                                                <Table.Td>
                                                                                    <SmartTimeDisplay
                                                                                        hours={op.realArrivalTime ?? op.RealArrivalTime}
                                                                                        baseDateStr={String(plan.planDate)}
                                                                                    />
                                                                                </Table.Td>

                                                                                <Table.Td>
                                                                                    <SmartTimeDisplay
                                                                                        hours={op.realDepartureTime ?? op.RealDepartureTime}
                                                                                        baseDateStr={String(plan.planDate)}
                                                                                    />
                                                                                </Table.Td>
                                                                                ...
                                                                                <Table.Td>
                                                                                    <Group gap={4}>
                                                                                        <IconCrane size={14} color="gray" />
                                                                                        <Text size="sm">{op.crane || op.Crane}</Text>
                                                                                    </Group>
                                                                                </Table.Td>

                                                                                <Table.Td>
                                                                                    {(op.departureDelay ?? op.DepartureDelay) > 0 ? (
                                                                                        <Badge color="red" size="sm">
                                                                                            +{op.departureDelay ?? op.DepartureDelay}h
                                                                                        </Badge>
                                                                                    ) : (
                                                                                        <Text size="sm" c="dimmed">
                                                                                            -
